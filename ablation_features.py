"""Test whether adding CWM's 13 feature fields helps the FM baseline."""
...
def build(mode):
    """Modes: base=5 current fields, item=add item fields, cwm13=all fields."""
    ...
    def raw(x):
        ...
        if mode in ('item','cwm13'): f += ve[1:]                              # Add music/type/upload
        if mode=='cwm13':            f += ue                                  # Add user-side fields
        return f
...
for mode,desc in [('base','5 fields (current kit)'),('item','+4 item fields = 9'),('cwm13','all 13 CWM fields')]:
