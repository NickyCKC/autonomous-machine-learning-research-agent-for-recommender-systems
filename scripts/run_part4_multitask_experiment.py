"""Run paired BPR-continuation and shared-bottom multi-task experiments."""
...
def main() -> int:
    ...
    parser.add_argument("--plan", default="configs/part4_multitask_nodes.json")
    parser.add_argument("--output", default="results/part4_multitask")
    parser.add_argument("--priorities", default="results/research_priorities.json")
    args = parser.parse_args()
    nodes = load_plan(args.plan)
    decision = DeterministicResearchPolicy(Path(args.priorities)).select(limit=3)
    summary = ExperimentRunner(build_track2_registry()).run(
        nodes, Path(args.output), policy_decision=decision
    )
